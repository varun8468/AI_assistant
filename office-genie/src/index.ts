...
// Also fix your ask endpoint to ensure consistent sessionId types
app.post("/ask", async (req, res) => {
  try {
    ...
    // Ensure sessionId consistency - convert to number if it's a numeric string
    let currentSessionId = sessionId;
    if (sessionId && !isNaN(parseInt(sessionId))) {
      currentSessionId = parseInt(sessionId);
    } else if (!sessionId) {
      // Generate a numeric sessionId to match your existing pattern
      currentSessionId = Math.floor(Math.random() * 1000000);
    }
    ...
    const memory = await getMemoryForSession(currentSessionId.toString());
...
// Update the getMemoryForSession function to handle both string and numeric sessionIds
async function getMemoryForSession(sessionId: string | number) {
  const collection = mongoClient.db("langchain").collection("memory");
  
  return new BufferMemory({
    chatHistory: new MongoDBChatMessageHistory({
      collection,
      sessionId: sessionId.toString(), // Always convert to string for MongoDBChatMessageHistory
    }),
    returnMessages: true,
  });
}
